...
public class ModMenu implements ModMenuApi {
    ...
    public ConfigScreenFactory getModConfigScreenFactory() {
        return parent -> {
            ...
            .addEntry(entryBuilder.startFloatSlider(V.translatableText("config.breadcrumbs.segment_length"), settings.segmentLength, 0f, 25f).setDefaultValue(1f).setSaveConsumer(v -> settings.segmentLength = v / 100f).build())
            ...
            .addEntry(entryBuilder.startEnumSelector(V.translatableText("config.breadcrumbs.trail_mode"), TrailMode.class, settings.trailMode).setDefaultValue(TrailMode.LINES).setSaveConsumer(v -> settings.setTrailMode(v)).build())
